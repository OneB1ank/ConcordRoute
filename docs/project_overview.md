...
本文说明 ConcordRoute 的产品责任、运行边界、仓库所有权和跨模块核心术语，供首次进入仓库或判断变更归属时建立共同上下文。本文不枚举具体 HTTP 字段、平台模型表、部署命令或单个函数行为；这些内容由对应分类文档或库外用户手册负责。
...
ConcordRoute 主要修改自 TokenFlux/TokenRouter，并沿其上游关系持续吸收 Wei-Shaw/Sub2API 的修复。调用方使用平台签发的 API Key 访问 Anthropic、OpenAI、Gemini 及其他兼容入口；平台负责认证、模型路由、上游账号调度、协议转换、故障转移、并发与限额控制、用量记录和计费。浏览器管理端同时提供用户自助、管理员运营、支付订阅、用量分析和运行观测能力。
