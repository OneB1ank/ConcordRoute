...
OpenAI OAuth 的 401 只有在上游明确返回 `token_invalidated` 或 `token_revoked`，或者账号缺少可用 `refresh_token` 时才进入长期错误。其余 401 无论是否绑定代理，都按失败请求观察到的凭据和代理绑定执行原子比较：状态未变化时失效 Token 缓存并进入正常 OAuth 刷新窗口；并发刷新、重新授权或改绑代理已经完成时忽略迟到的旧响应。
...
Codex 额度透支是纯调度策略：只对显式启用的 OAuth 账号绕过本地额度预留阈值，业务请求体、工具、instructions 和 metadata 保持原样。真实上游 429 继续写入标准 `RateLimitResetAt` 与运行时阻断，并解除已经失效的粘性绑定；认证错误、人工暂停、模型冷却、代理故障和过载也不会被透支开关绕过。后台额度刷新只读取 `/wham/usage` 元数据，不发送合成 `/responses` 模型请求。

## 运维诊断

- 观察每 provider 的候选数、刷新成功/失败、节流、超时和最长积压，而不只看总成功率。
- 关联账号测试、额度查询、凭据刷新、代理健康和调度过滤原因，区分凭据故障与出站网络故障。
