...
OpenAI OAuth 的 401 只有在上游明确返回 `token_invalidated` 或 `token_revoked`，或者非代理链路确认账号缺少可用 `refresh_token` 时才进入长期错误。配置代理的账号若收到通用 `{"detail":"Unauthorized"}`，只对当前账号设置一分钟出站冷却，不失效 Token 缓存；冷却到期后自动重新参与调度。直连账号的同形 401 仍会失效缓存并进入正常 OAuth 刷新窗口。
...
平台可维护独立的上游额度快照：OpenAI/Codex 窗口、Gemini tier/model quota、Antigravity credits、Grok 计费/媒体资格、Qoder Credits 等。快照用于调度、容量展示和诊断，不是 ConcordRoute 用户余额或订阅账本。
...
Codex 额度透支状态还区分“合成探针失败”和“真实业务请求收到额度 429”：后者以单次业务证据终止当前透支周期，固定记录 `attempts=1`，暂停账号至上游恢复时间，并解绑已失效的粘性会话。该终态仍保留透支起点及请求、Token、成本统计，管理端显示“透支后被上游 429 终止”，避免把探针次数误当作真实业务次数。真实额度窗口随后恢复时，`recovered` 可以覆盖同周期的业务 429 终态；其它 `passed` 或 `inconclusive` 写入仍不能覆盖该终态，避免并发旧探针抹掉有效结论。
