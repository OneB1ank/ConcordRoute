...
刷新成功后要原子更新凭据/过期时间，清理可恢复错误，并同步账号缓存与调度快照。创建、编辑、复制、导入、刷新和定时维护都不会自动调用 OpenAI/Antigravity 隐私设置接口；隐私策略只由管理员在账号详情中的“设置隐私”按钮显式触发。账号未设置 `privacy_mode` 时保持未设置，不因令牌交换、令牌刷新或后台维护而改变。刷新失败按失败阈值记录，不立即把一次瞬时网络错误等同于永久禁用；凭据明确撤销或账号归属失效时才进入需要重新授权的状态。
...
账号创建、编辑、复制、批量导入和批量更新不再自动向上游发起查活、配额或端点能力探测。这些操作只能由管理员明确请求的用量/配额接口、手动账号测试或显式调度计划触发。这不影响 OAuth 凭据到期时的按需刷新，也不影响本地账务、用量记录和 Redis 配额刷新。
...
管理员账号页的 usage 列是独立的展示刷新入口：页面挂载、列表刷新和前端缓存失效只读取账号 Extra、内存快照及本地账务统计，并通过 `/admin/accounts/usage/batch` 批量构建被动展示数据；该接口不会代表账号访问上游额度端点。对话响应产生的 Codex/平台响应头快照仍会更新账号状态，管理员若确需实时探测需点击单账号主动查询或使用明确的批量查询操作。前端约五分钟缓存只用于减少管理端重复读取，不改变上游探测策略。

OpenAI 账号 5h 行的请求数、Token、A/U 费用来自该账号全部已记录 `usage_logs`，
每次按 `[now-5h, now)` 重新求和，跨用户、Key 和会话累计；不按最新会话或上游 `reset_at` 截断，
也不把每次响应的额度快照重复相加。上游 `used_percent` 与重置时间继续独立展示和用于原有额度策略，
不会从本地 Token/费用反推百分比。7d 统计仍对齐上游周周期，无有效周周期时使用原有本地滚动回退。
两行共用统计终点，但起点不同：周窗口刚重置时，滚动 5h 仍含上一周末尾的账务，而 7d 只含新周账务，
因此 5h 的请求数、Token 和费用可能大于 7d；这不是求和遗漏，不应通过裁剪 5h 或把上周费用补进 7d 强行对齐。
前端保留统一的 `5h` / `7d` 标签，累计行的悬停说明解释两种口径，不重复添加“近5h”标签。
没有上游 5h 窗口的账号只展示本地统计，不补造额度条；用量异步落库、管理端缓存和历史记录清理仍会影响展示的时效及完整性。
