...
Codex 额度透支状态还区分“合成探针失败”和“真实业务请求收到额度 429”：后者以单次业务证据终止当前透支周期，固定记录 `attempts=1`，暂停账号至上游恢复时间，并解绑已失效的粘性会话。该终态仍保留透支起点及请求、Token、成本统计，管理端显示“透支后被上游 429 终止”，避免把探针次数误当作真实业务次数。
