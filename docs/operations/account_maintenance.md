...
OpenAI OAuth 的 401 只有在上游明确返回 `token_invalidated` 或 `token_revoked`，或者账号缺少可用 `refresh_token` 时才进入长期错误。代理节点返回的通用 `{"detail":"Unauthorized"}` 只触发 token 缓存失效和临时不可调度，等待刷新或链路恢复；不能仅凭这一种响应形状永久禁用账号。
