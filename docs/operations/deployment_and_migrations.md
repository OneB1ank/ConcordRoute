...
本文记录 ConcordRoute 构建产物、运行拓扑、首次初始化、数据库迁移、升级和恢复之间的工程边界。逐命令安装步骤由既有部署手册维护；修改启动装配、镜像、Compose、setup、SQL 迁移或在线更新前应先读本文。
...
迁移 `279_restore_live_usage_request_type.sql` 修复部分历史升级在218之后补执行195导致的枚举收窄，重新允许 `request_type=5`。它不改写使用记录、费用、身份或账号配置，使用 `NOT VALID` 避免扫描历史用量大表，同时对新增和更新记录立即检查0至5；负值和大于5的未知类型仍被拒绝。DDL锁等待最多5秒，失败会随事务回滚并阻止带着错误schema启动，应排查长事务后重试。

该放宽与旧二进制兼容；回退应用时保留放宽的约束，不删除迁移记录，也不收窄或删除已写入的Live记录。升级验收需检查约束定义与新增Live记录实际落库，不能只检查迁移表存在218的记录。Live正式扣费尚未接入，枚举修复不表示零费用记录已成为有效计费。
