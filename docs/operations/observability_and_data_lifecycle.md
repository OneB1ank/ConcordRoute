...
OpenAI 原生 Responses HTTP/WS 的新使用记录优先使用首语义事件时间：空推理结构、合法空
part 或字符串型空 delta 也可能触发，不等于实际文字到达。转发结果内的真实首内容、
`first_content_received`/Flush 诊断和账号调度反馈保持原口径；只在 `RecordUsage` 入库边界
选取展示值，不新增数据库列，也不增加热路径数据库读取。历史记录不回填，其它平台/协议沿用原值。
使用记录导出、监控及预聚合读取该字段，跨版本比较应注意口径变化，不能将数字下降当成传输提速。
