...
本文总览 ConcordRoute 的进程日志、Ops 数据、用量记录、计费事实、审计、聚合、清理、备份和数据共享如何产生、保存和失效。具体监控告警、预聚合和数据共享机制由独立专题拥有；修改采集队列、运维查询、保留策略、备份范围或导出流程前应先由本文路由。
...
开启 `gateway.ttft_diagnostics_enabled` 时，HTTP Responses/透传/原始 Chat 的 access log
通过 `ttft_attempts` 分别记录各次上游 Do 的编号、账号与阶段时间，最多保留最近 64 次。
平面 `ttft_stages_ms` 的流阶段只来自最后一次尝试，请求前置阶段保留首次值。
首内容解析、开始 flush、完成 flush 独立记录，避免把下游背压判为上游等待；
网关 Flush 时间不是客户端端到端首字。字段与归因边界见[首字指南](../guides/first-token-latency.md)。

OpenAI 原生 Responses HTTP/WS 的新使用记录优先使用首响应（首块）时间：本次转发开始至
首个非空上游正文块/应用消息，创建通知也计入；网关本地产生的心跳不计入。
这不是实际文字或客户端收取时间。真实首内容、`first_content_received`/Flush 诊断和调度反馈
保持原口径。WS passthrough 展示总耗时也对齐新首响应的请求起点，内部旧 Duration 不变。
只在 `RecordUsage` 入库边界选取展示值，不新增数据库列或热路径数据库读取。
历史不回填，其它平台/协议沿用原值。导出、监控及预聚合也受展示口径影响；数值下降不等于传输提速。
完整边界及验证见[首字指南](../guides/first-token-latency.md)。

使用记录的首字空值不等同于 0ms，也不由总耗时回填。明确非流式且没有独立首内容样本时，
界面显示“不适用”；流式、WS 或未知类型缺少样本时显示“未记录”并解释历史缺失、
流中断或没有符合当前口径的事件的可能性。已记录的 0ms 仍展示数值。完整拒绝内容与 incomplete
终态里的非空正文也属于首内容；状态、用量计数及加密推理本身不用于推定实际内容出现时间。

首字诊断还通过 `ttft_operations` 保存重复的选择、槽位、token 获取/锁等待和重试操作，
在 `ttft_attempts[].transport` 中保存每次 Do 的连接与写出事件。每个事件列表最多 128 条，
超量显式记录 `dropped`；只包含固定阶段名、相对毫秒、失败标志和连接复用元数据。
关闭诊断保持无采样状态；不记录额外地址、Header、正文或凭据，不主动访问上游。
诊断仍遵守访问日志输出与落库开关，完整判读边界见[首字延迟指南](../guides/first-token-latency.md)。

入口诊断另外区分 `api_key_auth`（API Key 认证/准入，结束于调用后续 handler 前）、
`request_body_read`（一次请求体读取及解压，不包含后续 JSON 规范化），
以及 `identity_lock_wait`、`identity_binding_read/merge/write`。
身份阶段保留重试事件，写入哈希命中时没有 write 事件，不伪造零耗时写入。
认证内部若读取请求体，两段属于嵌套关系，应按起止区间解释，不重复相加。
这些固定阶段仍由同一个默认关闭的诊断开关控制，不采集正文、凭据、完整绑定或动态错误文本。

诊断事件在请求内存中采样，不逐事件写日志。固定阶段名复用常量，避免流式 Flush 重复标记
制造临时字符串；只有允许输出访问日志或 Gin 错误日志时才构造结束快照。
标准输出/文件日志仍遵循既有同步写出行为，不把 Ops 异步落库等同于所有日志链路无阻塞。
本机基准与阻塞日志端测试覆盖这些开销边界，长期全量开启仍需要评估 CPU、GC 和日志吞吐。
...
响应，本列保持未记录，不复用连接握手状态冒充本回合的新响应。失败尝试不写入最终成功
...
迁移 `281_usage_upstream_observation.sql` 只新增两个可空列，无索引、默认值或历史回填；
