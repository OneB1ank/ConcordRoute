...
`GET /api/v1/admin/groups/live-capability` 的 `supported`、`server_supported` 与 `live_transport_supported` 表示服务实现了 WebRTC 创建与 Sideband 转发，不表示特定账号或真实通话已验收。`attestation_policy=if_available` 表示网关有真实来源时附带证明，无来源时省略，由上游判定是否接受。`attestation_source_available` 单独表示本地 Provider 检查通过或至少一条活动 app-server bridge 完成协商；断开全部 bridge 且没有本地 Provider 时恢复 `false`，不影响传输能力。管理页每次开启 Live 时重新检查服务能力；查询失败或未声明支持时提示确认，仍允许管理员预先开启分组权限。

`live_devicecheck_server` 表示本地 Provider 的 `Check` 已通过；Linux 的检查仅验证 helper 文件路径、权限和属主，不调用 `Generate`，也不向上游发探测请求。`server_attestation_provider` 会标识 `macos_devicecheck`、`linux_external_helper` 或 `none`。Linux helper 只桥接外部真实证明，网关自身不生成 token。`live_attestation_mode` 按当前来源返回 `client_relay`、`server_provider`、`server_and_client_relay` 或 `none`；本地 Provider 的检查错误单独保留在 `server_reason`。提供器存在但损坏、生成失败、证明格式异常或 Sideband 密文解密失败时仍返回明确错误，不伪造成功、不静默丢弃已有证明。

客户端可通过独立的 `GET /backend-api/codex/app-server` WebSocket 协商，`app_server_attestation_transport` 只描述本进程活动连接。Live 还保留客户端直接携带真实 `x-oai-attestation` 的兼容路径；该入口检查 UA/originator 配对和 envelope 格式，不应把这些检查等同于真实性验证。真实证明的接受情况以对应上游请求为准；UA/TLS 模板不是设备证明。客户端直传能力不表示管理接口能预知下一笔请求是否附带有效证明。
...
Clash 策略的批量出口绑定使用 `POST /api/v1/admin/proxies/clash/profiles/:id/bind-openai-oauth`。目标策略必须处于运行态且已经生成 managed proxy；服务端只选择未删除、非影子、`proxy_id IS NULL` 的 OpenAI OAuth 账号，保留已有自定义代理。响应返回 `eligible`、`bound`、`failed` 和可选逐账号 `failures`，单项账号更新失败会回滚该账号的 binding，但不撤销本批已经完成的其它账号。
