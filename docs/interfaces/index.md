...
- [听写转录](audio_transcription.md)：录音上传、桌面别名、音频计量、独立音频宿主、分段听写及本机朗读。读取时机：修改转录路由、STT 定价、Windows 音频接线、短期能力或客户端认证对接时读取。
...
- [插件宿主接口](plugin_host.md)：插件包信任、管理员生命周期、路由绑定、Host API、隔离运行和数据库迁移边界。读取时机：安装插件宿主、修改插件管理 API、隔离策略或插件与网关的接线边界时读取。
