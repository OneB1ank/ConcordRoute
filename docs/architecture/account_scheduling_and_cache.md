...
完整账号与筛选 metadata 使用独立的 `v2` 缓存命名空间，避免滚动升级期间旧进程的普通 SET 覆盖新进程的版本保护。
两份载体及账号版本在同一次 Redis Lua 操作中发布；版本较旧的单账号同步和桶重建均不覆盖新值。
账号版本取数据库 `updated_at` 的 UTC 定长精度表示；迁移 278 的行级触发器在更新时至少递增一微秒，
同事务内多次更新、较早启动事务的延迟写入也得到严格递增的版本。更新顺序由账号行锁决定，不依赖应用服务器时钟。
相等版本允许幂等重发及关联字段刷新，空版本仅兼容旧适配器，不能覆盖已有真实版本。
账号删除保留墓碑，延迟账号列表不复活该 ID；编码失败只执行有版本保护的缓存驱逐，不产生删除墓碑。
这项保护针对仍保留版本下界的缓存；Redis 整体丢失或手动清除版本状态后由数据库重建恢复。

OpenAI 基础调度完成数据库资格复核后，在请求内标记该完整快照来源，最终选择直接沿用它，不再回读旧缓存覆盖。
标记不序列化到 Redis 或 API，仓储共享对象不被原地打标；普通筛选投影仍按原规则水合。
首次使用 v2 key 会发生缓存冷读与重建；全量切换后旧命名空间可按运维流程清理，回退旧二进制时先重建旧缓存。
这些调整不改变账号身份绑定内容、客户端会话种子或 Cockpit 算法，也不构成跨实例绑定集合原子合并保证。
...
Codex 用量快照同时保存 `codex_5h_available` 与 `codex_7d_available`。明确不存在的上游窗口不会参与进度展示、自动暂停、调度余量或透支判断；旧快照缺少标记时才按已有 canonical/legacy 字段兼容读取。账号只有周窗口时，本地账务仍可按独立的最近 5 小时范围展示统计，但该行标记为本地统计，不显示伪造的百分比或重置时间。5 小时与 7 天统计范围各自使用对应上游重置边界，周窗口刚重置时不得为了嵌套关系把范围扩展到旧的 5 小时周期。
