...
const PLATFORMS: PlatformQuotaPlatform[] = ['anthropic', 'openai', 'gemini', 'antigravity', 'qoder', 'grok']
...
async function onSave() {
  if (!props.user) return
  // 提交前拒绝负数和非有限数，避免 normalizeLimit 将非法额度静默转成 null（无限额）。
  // 保留 0（禁用）与清空输入（无限额）的既有语义。
