...
// Usage
  usage: {
    ...
    upstreamStatus: '上游状态',
    upstreamStatusHint: '最终上游 HTTP 响应的实测状态码；state 只展示响应头字符串长度，不区分套餐或状态。不是客户端收到的状态码。',
    upstreamStatusMissingHint: '历史记录、无逐请求 HTTP 响应的 WebSocket 或未接入路径未采集；不按 200 补值。',
    codexTurnStateHint: '仅统计上游响应头 x-codex-turn-state 去除外侧空白后的字节数，不表示请求是否携带 state，也不表示插件是否改写请求。未返回仅指响应缺少该值；长度不推断套餐、满血或降级。',
    codexTurnStateAbsent: '响应 state · 未返回',
    codexTurnStatePresent: '响应 state · {bytes} B',
