...
// API Keys
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description: '配置 Grok Build、Claude Code、Codex 或 OpenCode，让请求通过当前 ConcordRoute Grok 分组发送。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 ConcordRoute Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 ConcordRoute Grok 分组发送。',
        ...
        codexNote: '将 config.toml 保存到 ~/.codex，并在启动 Codex 前设置 CONCORDROUTE_API_KEY。',
        codexNoteWindows: '将 config.toml 保存到 %USERPROFILE%\\.codex，并在 PowerShell 中设置 CONCORDROUTE_API_KEY 后启动 Codex。'
      },
      opencode: {
        title: 'OpenCode 配置示例',
...
// Usage
  usage: {
    ...
    upstreamStatus: '上游状态',
    upstreamStatusHint: '最终上游 HTTP 响应的实测状态码；Codex 正常态与降级态通常都返回 HTTP 200，实际判据是下方 Turn-State 长度。不是客户端收到的状态码。',
    upstreamStatusMissingHint: '历史记录、无逐请求 HTTP 响应的 WebSocket 或未接入路径未采集；不按 200 补值。',
    codexTurnStateHint: '响应头 x-codex-turn-state 去除外侧空白后的字符串长度；Pro 为 292 满血、312 降级，Team 为 332 满血、356 降级。仅保存长度，不保存原文。',
    codexTurnStateAbsent: 'state · 未返回',
    codexTurnStateFull: 'state · {bytes} B · {plan} 满血',
    codexTurnStateDegraded: 'state · {bytes} B · {plan} 降级',
    codexTurnStateUnknown: 'state · {bytes} B · 未知',
    codexTurnStateRequestModeHint: '请求侧实际处理：已注入表示本次请求使用了缓存的满血 State；采集表示本次走获取 State 阶段；未启用表示账号未开启注入；未记录表示历史或未接入路径。',
    codexTurnStateRequestInjected: '请求 · 已注入',
    codexTurnStateRequestAcquire: '请求 · 采集',
    codexTurnStateRequestDisabled: '请求 · 未启用',
    codexTurnStateRequestNotRecorded: '请求 · 未记录',
    codexTurnStateRequestUnknown: '请求 · 未知',
    ...
    latencyFirstTokenNotRecordedHint: '未记录到首字样本，可能是历史统计缺失、流中断或没有符合当前口径的输出事件；不代表零延迟。',
    latencyDuration: '总耗时',
    time: '时间',
    ws: 'WS',
    stream: '流式',
