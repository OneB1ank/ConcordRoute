...
// Settings
    settings: {
      ...
      features: {
        channelMonitor: {
          title: '渠道监控',
          description: '默认使用 V2 被动聚合；也可显式切换到 V1 主动探测。两种模式互斥运行，关闭后停止后台任务并隐藏用户入口。',
...
// TLS 指纹模板
    tlsFingerprintProfiles: {
      ...
      form: {
        ...
        yamlParsed: 'YAML 解析成功，字段已自动填充',
        yamlParseFailed: 'YAML 解析失败：未找到 name 字段',
        yamlAlpnInvalid: 'YAML 中的 alpn_protocols 应为数组（例如 ["h2", "http/1.1"]）、null 或空值',
        name: '模板名称',
        ...
        rustlsNativeOrder: '启用 rustls 原生排序',
        rustlsNativeOrderHint: '开启后优先按 rustls 0.23.36 算法排列 ClientHello 扩展，每条新连接使用独立种子，握手重试新增 Cookie 时复用该种子；关闭则保持模板顺序。不改变 UA 或业务会话。需关闭 GREASE；这不是切换整个 TLS 后端。',
        rustlsNativeOrderConflict: '原生排序需关闭 GREASE 并移除 GREASE 扩展；暂不支持手动声明 PSK(41)、Cookie(44) 或真实 ECH outer_extensions(64768)。',
        cipherSuites: '密码套件',
        ...
        alpnProtocolsHint: '逗号分隔，例如 h2, http/1.1。无 ALPN 实采请留空，并保留不含 0x0010 的自定义扩展列表；扩展列表留空时使用默认握手配置。',
        alpnExtensionConflict: '已填写 ALPN 协议，但自定义扩展列表缺少 0x0010（16）。请选择修复方式后保存：补齐扩展会改变握手；若保留无 ALPN 实采，请清空协议。',
        addAlpnExtension: '补齐 ALPN 扩展 16',
        clearAlpnProtocols: '清空 ALPN，保持无 ALPN',
    ...
    codexAttestationCollector: {
      title: 'Codex app-server 证明采集器',
      description: '仅采集真实 Windows Codex 客户端的能力协商与 opaque proof 摘要，不保存证明原文。',
      scopeTitle: '作用范围',
      scopeAttestation: '证明层：initialize / generate 摘要',
      scopeIdentity: '身份层：沿用账号 Codex 收敛',
      scopeTransport: '传输层：沿用账号 TLS 模板/路由',
      running: '运行中',
      stopped: '已停止',
      start: '启动',
      stop: '停止',
      ...
      token: '采集令牌',
      expiresAt: '过期时间',
      bridgeQuery: 'app-server 查询参数',
      bridgeEndpoint: 'WebSocket 地址（HTTP/1.1 Upgrade）',
      headerHint: '或使用握手头',
      copyEndpoint: '复制地址',
      deleteSession: '删除会话',
      sessionDeleted: '采集会话已删除',
      deleteSessionFailed: '删除采集会话失败',
      captures: '采集结果',
      noCaptures: '暂无结果。将 collector_token 附加到 app-server WebSocket（HTTP/1.1 Upgrade）后运行客户端。',
      client: '客户端',
      connection: '连接',
      session: '会话',
      proof: '证明摘要',
      statusFailed: '加载证明采集器状态失败',
      actionFailed: '证明采集器操作失败',
      sessionFailed: '创建证明采集会话失败',
      copied: '采集头已复制',
      copyFailed: '复制失败'
    },
}
