...
// Accounts
    accounts: {
      ...
      usageWindow: {
        ...
        localFiveHourStatsHint: 'Local requests, tokens, and A/U costs across all sessions in the last 5 hours, independent of upstream quota utilization and reset time. Just after a weekly reset, this still includes the previous cycle’s final hours and may exceed the current 7d cycle total.',
        localSevenDayStatsHint: 'Local requests, tokens, and A/U costs within the current upstream weekly cycle, falling back to the last 7 days when no valid cycle is available. Records before a weekly reset are excluded from the new cycle, so its total may be lower than the last 5 hours. Utilization comes from upstream, not from local costs.',
