...
func (h *ChannelMonitorV2Handler) Dimensions(c *gin.Context) {
	...
	filter, ok = h.scopeFilter(c, filter)
	if !ok {
		return
	}
...
// scopeFilter 在普通用户查询进入仓储层前施加分组权限边界。
func (h *ChannelMonitorV2Handler) scopeFilter(c *gin.Context, filter service.ChannelMonitorV2Filter) (service.ChannelMonitorV2Filter, bool) {
	subject, ok := middleware.GetAuthSubjectFromContext(c)
	if !ok || subject.UserID <= 0 {
		response.Unauthorized(c, "user not found in context")
		return service.ChannelMonitorV2Filter{}, false
	}
	scoped, err := h.service.ScopeFilterForViewer(
		c.Request.Context(),
		filter,
		subject.UserID,
		channelMonitorV2IsAdmin(c),
	)
	if err != nil {
		response.ErrorFrom(c, err)
		return service.ChannelMonitorV2Filter{}, false
