...
func TestPublicSettingsInjectionPayload_SchemaDoesNotDrift(t *testing.T) {
	...
	dtoOnlyFields := map[string]string{
		...
		// Cyber 会话屏蔽仅供管理员配置，普通用户页面不消费该内部风控参数。
		"cyber_session_block_enabled":     "admin-only risk-control setting",
		"cyber_session_block_ttl_seconds": "admin-only risk-control setting",
	}
