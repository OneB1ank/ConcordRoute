...
func (s *OpenAIGatewayService) forwardOpenAIWSV2(
	...
	if account.Type == AccountTypeOAuth {
		applyCodexVersionClientMetadata(payload, codexVersionFromOutboundHeaders(wsHeaders))
	}
	logOpenAIWSModeDebug(
	...
	if handshakeTurnState != "" {
		if stateStore != nil && sessionHash != "" {
			stateStore.BindSessionTurnState(groupID, sessionHash, handshakeTurnState, s.openAIStickySessionTTL())
		}
		if c != nil {
	...
	if stateStore != nil && storeDisabled && sessionHash != "" {
		stateStore.BindSessionConn(groupID, sessionHash, lease.ConnID(), s.openAIStickySessionTTL())
	}
	firstTokenMsValue := -1
	if firstTokenMs != nil {
	...
	return &OpenAIForwardResult{
		...
		UpstreamResponseServiceTier: observedOpenAIServiceTierFromPayload(finalResponse),
