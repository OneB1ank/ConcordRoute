...
// Gin 模式是全局变量，所有并行用例都在 t.Parallel 暂停前完成初始化。
...
func TestForwardAsAnthropic_TrimsFullReplayOnlyForCodexCompatModels(t *testing.T) {
	...
	// 历史裁剪策略保持不变，输出只包含原始消息。
	require.Equal(t, int64(openAICompatAnthropicReplayMaxTailMessages), gjson.GetBytes(codexBody, "input.#").Int())
	require.NotContains(t, string(codexBody), openAICompatClaudeCodeTodoGuardMarker)
	require.Equal(t, "message-03", gjson.GetBytes(codexBody, "input.0.content.0.text").String())
	require.Equal(t, "message-14", gjson.GetBytes(codexBody, "input.11.content.0.text").String())
...
func TestForwardAsAnthropic_AttachesPreviousResponseIDForCompatContinuation(t *testing.T) {
	...
	require.NotContains(t, string(upstream.lastBody), openAICompatClaudeCodeTodoGuardMarker)
	require.Equal(t, "second", gjson.GetBytes(upstream.lastBody, "input.0.content.0.text").String())
}
...
func TestForwardAsAnthropic_PreviousResponseIDKeepsMultiToolCallContext(t *testing.T) {
	...
	// 去掉额外 developer 提示后，两个调用及结果仍保持完整顺序和对应关系。
	require.Equal(t, int64(5), gjson.GetBytes(upstream.lastBody, "input.#").Int())
	require.NotContains(t, string(upstream.lastBody), openAICompatClaudeCodeTodoGuardMarker)
	require.Equal(t, "function_call", gjson.GetBytes(upstream.lastBody, "input.0.type").String())
	require.Equal(t, "call_one", gjson.GetBytes(upstream.lastBody, "input.0.call_id").String())
	require.Equal(t, "function_call", gjson.GetBytes(upstream.lastBody, "input.1.type").String())
	require.Equal(t, "call_two", gjson.GetBytes(upstream.lastBody, "input.1.call_id").String())
	...
	require.Equal(t, "continue", gjson.GetBytes(upstream.lastBody, "input.4.content.0.text").String())
}
...
func TestForwardAsAnthropic_ReplaysWithoutContinuationWhenPreviousResponseMissing(t *testing.T) {
	...
	require.NotContains(t, string(upstream.bodies[1]), openAICompatClaudeCodeTodoGuardMarker)
	require.Equal(t, "first", gjson.GetBytes(upstream.bodies[1], "input.0.content.0.text").String())
	require.Equal(t, "second", gjson.GetBytes(upstream.bodies[1], "input.2.content.0.text").String())
}
...
func TestForwardAsAnthropic_APIKeyMetadataSessionSurvivesChangingCacheControlAnchorAfterContinuationDisabled(t *testing.T) {
	...
	require.False(t, gjson.GetBytes(upstream.bodies[1], "previous_response_id").Exists())
	require.Equal(t, int64(openAICompatAnthropicReplayMaxTailMessages+4), gjson.GetBytes(upstream.bodies[1], "input.#").Int())
	require.NotContains(t, string(upstream.bodies[1]), openAICompatClaudeCodeTodoGuardMarker)
	require.Equal(t, "rewritten context", gjson.GetBytes(upstream.bodies[1], "input.0.content.0.text").String())
	require.Equal(t, "message-15", gjson.GetBytes(upstream.bodies[1], "input.15.content.0.text").String())
}
...
func TestForwardAsAnthropic_OAuthRestoresCodexIdentityHeaders(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			userAgent:      tuiUA,
			originator:     "opencode",
			wantUserAgent:  "codex-tui/0.153.4 (Mac OS X 15.6; arm64) Terminal.app (codex-tui; 0.153.4)",
...
func TestForwardAsAnthropic_OAuthCockpitMessagesAlignsFingerprintHeaders(t *testing.T) {
	t.Parallel()

	upstream := &httpUpstreamRecorder{resp: openAICompatSSECompletedResponse("resp_oauth_cockpit", "gpt-5.4")}
	svc := &OpenAIGatewayService{
		httpUpstream: upstream,
		cfg:          &config.Config{Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{Enabled: false}}},
	}
	account := newTestOAuthAccount(7001, map[string]any{
		codexFingerprintModeExtraKey: "cockpit",
		"openai_device_id":           "client-installation",
	})
	account.Credentials = map[string]any{
		"access_token":       "oauth-token",
		"chatgpt_account_id": "chatgpt-acc",
	}
	require.Equal(t, codexFingerprintCockpit, account.GetCodexFingerprintMode())
	...
	c.Request.Header.Set("x-client-request-id", "per-request-id")
	require.False(t, isOpenAIResponsesCompactPath(c))
	preIDs := resolveCodexFingerprintIDsFromRequest(account, c.Request.Header, map[string]any{"prompt_cache_key": "stable-cache"})
	require.NotNil(t, preIDs)

	result, err := svc.ForwardAsAnthropic(context.Background(), c, account, body, "stable-cache", "gpt-5.4")
	require.NoError(t, err)
	require.NotNil(t, result)

	require.Equal(t, upstream.lastReq.Header.Get("session_id"), upstream.lastReq.Header.Get("session-id"))
...
func TestForwardAsAnthropic_OAuthPreservesMessagesWithoutTodoGuard(t *testing.T) {
	t.Parallel()
	...
	require.Equal(t, "developer", gjson.GetBytes(upstream.lastBody, "input.0.role").String())
	require.Equal(t, "project instructions", gjson.GetBytes(upstream.lastBody, "input.0.content.0.text").String())
	// 原始系统提示提升为 developer 后，直接保留用户消息，不插入任务管理指令。
