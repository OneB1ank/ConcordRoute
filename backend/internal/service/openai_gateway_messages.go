...
func (s *OpenAIGatewayService) ForwardAsAnthropic(
	...
	if account.Type == AccountTypeOAuth && account.Platform != PlatformGrok {
		...
		enforceCodexIdentityHeadersWithUA(upstreamReq.Header, s.codexIdentityOverrideUA(account, tlsRouterMatch...))
		match := TLSFingerprintRouterMatchResult{}
		if len(tlsRouterMatch) > 0 {
			match = tlsRouterMatch[0]
		}
		s.rememberOpenAIOutboundIdentity(account, upstreamReq.Header.Get("User-Agent"), match)
