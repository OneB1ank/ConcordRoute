...
// 移植插件宿主不应自动继承其他项目的发布者信任根。
func TestPluginPublisherTrustRequiresExplicitConfiguration(t *testing.T) {
	cfg := testPluginConfig(t.TempDir(), false)
	const legacyKey = "sub2api-openai-transport-v1"
	require.Empty(t, trustedPluginPublisherKey(nil, legacyKey))
	require.Empty(t, trustedPluginPublisherKey(cfg, legacyKey))
	publicKey, _, err := ed25519.GenerateKey(rand.Reader)
	require.NoError(t, err)
	encodedKey := base64.StdEncoding.EncodeToString(publicKey)
	cfg.Plugins.TrustedPublishers[legacyKey] = encodedKey
	require.Equal(t, encodedKey, trustedPluginPublisherKey(cfg, legacyKey))
}
