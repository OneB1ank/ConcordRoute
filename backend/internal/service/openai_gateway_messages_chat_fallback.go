...
func (s *OpenAIGatewayService) forwardAnthropicViaRawChatCompletions(
	...
	resp, err := s.sendCCUpstreamRequest(ctx, c, account, targetURL, chatBody, upstreamModel, clientStream, apiKey, account.GetOpenAIUserAgent(), "", tlsRouterMatch...)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) bufferChatCompletionsAsAnthropic(
	...
	return &OpenAIForwardResult{
		...
		UpstreamResponseServiceTier: normalizeObservedOpenAIServiceTier(ccResp.ServiceTier),
		Stream:                      false,
...
func (s *OpenAIGatewayService) streamChatCompletionsAsAnthropic(
	...
	if scan.Err != nil {
		...
		return &OpenAIForwardResult{
			...
			ReasoningEffort:             reasoningEffort,
			ServiceTier:                 serviceTier,
			UpstreamResponseServiceTier: scan.UpstreamResponseServiceTier,
			Stream:                      true,
			Duration:                    time.Since(startTime),
