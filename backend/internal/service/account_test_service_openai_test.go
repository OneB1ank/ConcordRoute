...
`))
	...
	require.NoError(t, err)
	require.Len(t, upstream.requests, 1)
	require.Equal(t, "codex-tui/0.145.0 test-terminal", upstream.requests[0].Header.Get("User-Agent"))
	require.Equal(t, "codex-tui", upstream.requests[0].Header.Get("Originator"))
	require.Equal(t, "0.145.0", upstream.requests[0].Header.Get("Version"))
}
