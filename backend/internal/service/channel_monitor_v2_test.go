...
type channelMonitorV2GroupAccessStub struct {
	groups []Group
	err    error
}

func (s channelMonitorV2GroupAccessStub) GetAvailableGroups(context.Context, int64) ([]Group, error) {
	return s.groups, s.err
}

func TestChannelMonitorV2ScopeFilterForViewer(t *testing.T) {
	svc := NewChannelMonitorV2Service(nil)
	svc.SetGroupAccessReader(channelMonitorV2GroupAccessStub{groups: []Group{{ID: 2}, {ID: 4}}})

	scoped, err := svc.ScopeFilterForViewer(context.Background(), ChannelMonitorV2Filter{GroupIDs: []int64{1, 2, 3}}, 9, false)
	require.NoError(t, err)
	require.True(t, scoped.GroupScopeRestricted)
	require.Equal(t, []int64{2}, scoped.GroupIDs)

	empty, err := svc.ScopeFilterForViewer(context.Background(), ChannelMonitorV2Filter{GroupIDs: []int64{1}}, 9, false)
	require.NoError(t, err)
	require.True(t, empty.GroupScopeRestricted)
	require.Empty(t, empty.GroupIDs)

	admin, err := svc.ScopeFilterForViewer(context.Background(), ChannelMonitorV2Filter{}, 9, true)
	require.NoError(t, err)
	require.False(t, admin.GroupScopeRestricted)
}
