...
// 生产 provider 同时连接账号目录、KV 和网关，避免仅修改 Wire 生成文件。
func TestProvidePluginManager_WiresHostAndGateway(t *testing.T) {
	...
	require.Same(t, manager, gateway.pluginManager)
	require.False(t, manager.ShouldRouteOpenAIOAuth(&Account{Platform: PlatformOpenAI, Type: AccountTypeOAuth}))
}
