...
type ChannelMonitorV2Service struct {
	...
	groupAccess channelMonitorV2GroupAccessReader
	now         func() time.Time
}

// channelMonitorV2GroupAccessReader 返回普通用户实际可绑定的分组，
// 用户侧监控查询必须与同一套分组授权规则保持一致。
type channelMonitorV2GroupAccessReader interface {
...
// SetGroupAccessReader 注入用户分组授权读取器。
func (s *ChannelMonitorV2Service) SetGroupAccessReader(r channelMonitorV2GroupAccessReader) {
	if s == nil {
		return
	}
	s.groupAccess = r
...
// ScopeFilterForViewer 将用户请求限制在其实际可绑定分组内。
// 管理员保留全局视图；普通用户在授权读取器缺失时按无分组权限处理。
func (s *ChannelMonitorV2Service) ScopeFilterForViewer(ctx context.Context, filter ChannelMonitorV2Filter, userID int64, admin bool) (ChannelMonitorV2Filter, error) {
	if admin {
		return filter, nil
	}
	filter.GroupScopeRestricted = true
	if s == nil || s.groupAccess == nil || userID <= 0 {
		filter.GroupIDs = nil
		return filter, nil
	}
	groups, err := s.groupAccess.GetAvailableGroups(ctx, userID)
	if err != nil {
		return ChannelMonitorV2Filter{}, err
	}
	allowed := make([]int64, 0, len(groups))
	for _, group := range groups {
		if group.ID > 0 {
			allowed = append(allowed, group.ID)
		}
	}
	allowed = normalizeInt64Set(allowed)
	if len(filter.GroupIDs) > 0 {
		allowed = intersectInt64Sets(allowed, filter.GroupIDs)
	}
	filter.GroupIDs = allowed
	return filter, nil
}

func intersectInt64Sets(left, right []int64) []int64 {
	if len(left) == 0 || len(right) == 0 {
		return nil
	}
	wanted := make(map[int64]struct{}, len(right))
