...
func TestOpenAIWSHTTPBridgeSessionIsolationAcrossSameSessionHash(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		c := newContext(r)
		// 从本 fork 的实际入口验证两条桥接连接，保持独立生命周期。
		hooks := &OpenAIWSIngressHooks{ClientLifecycleContext: ctx}
		serverResults <- svc.ProxyResponsesWebSocketFromClient(ctx, c, conn, account, "test-token", firstMessage, hooks)
	}))
	...
	for _, id := range []string{"alpha", "beta"} {
		...
		payload := fmt.Sprintf(`{"type":"response.create","model":"gpt-5","prompt_cache_key":%q,"client_metadata":{"thread_id":%q},"input":%q}`, "shared-cache", "shared-thread", id)
