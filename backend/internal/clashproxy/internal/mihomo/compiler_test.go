...
func TestCompileGeneratesPinnedLocalRuntimeConfig(t *testing.T) {
	...
	require.True(t, ok)
	require.Equal(t, "tr-clash-node-34", firstProxy["name"])
	...
	require.True(t, ok)
	require.Equal(t, "TR-CLASH-12", group["name"])
...
func TestCompileUsesInternalNodeNamesOnly(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, string(compiled), "tr-clash-node-7")
	require.NotContains(t, string(compiled), "User Clash Name")
}

func TestCompileMapsHTTPSAndShadowsocksToMihomoTypes(t *testing.T) {
	compiled, err := Compile(Profile{ID: 10, Strategy: proxyprofile.StrategySelect}, []proxynode.Node{
		{
			ID:     1,
			Type:   proxynode.TypeHTTPS,
			Config: map[string]any{"server": "https.example.com", "port": 443},
		},
		{
			ID:     2,
			Type:   proxynode.TypeSS,
			Config: map[string]any{"server": "ss.example.com", "port": 8388},
			Secret: map[string]any{"cipher": "aes-128-gcm", "password": "secret"},
		},
	}, RuntimeConfig{MixedPort: 17010, ControllerPort: 18010, ControllerSecret: "secret"})
	require.NoError(t, err)
