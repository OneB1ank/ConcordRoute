...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		codexTurnStateBytes       sql.NullInt64
		sessionID                 sql.NullString
		createdAt                 time.Time
		codexTurnStateRequestMode sql.NullString
	)
	...
	if upstreamStatusCode.Valid {
		...
		log.UpstreamStatusCode = &v
	}
	if codexTurnStateBytes.Valid {
		v := int(codexTurnStateBytes.Int64)
		log.CodexTurnStateBytes = &v
	}
	if sessionID.Valid {
		log.SessionID = &sessionID.String
	}
	if codexTurnStateRequestMode.Valid {
		log.CodexTurnStateRequestMode = &codexTurnStateRequestMode.String
	}
