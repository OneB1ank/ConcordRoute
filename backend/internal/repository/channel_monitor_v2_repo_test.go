...
func TestChannelMonitorV2WhereDeniesEmptyRestrictedGroupScope(t *testing.T) {
	filter := service.ChannelMonitorV2Filter{
		Start: time.Unix(1, 0), End: time.Unix(2, 0), GroupScopeRestricted: true,
	}
	cfg := service.ChannelMonitorV2Config{
		Platforms: []service.ChannelMonitorV2PlatformConfig{{Platform: "openai", Enabled: true}},
	}
	...
	catalog := channelMonitorV2CatalogFilter(service.ChannelMonitorV2Filter{
		GroupIDs: []int64{2, 4}, GroupScopeRestricted: true,
	})
	require.True(t, catalog.GroupScopeRestricted)
	require.Equal(t, []int64{2, 4}, catalog.GroupIDs)
}
...
func TestChannelMonitorV2UsageSuccessExcludesCyberBillingRows(t *testing.T) {
	...
	// 当前分组模型没有上游的 composite 平台占位值；按实际分组平台优先，
	// 分组平台为空时回退账号平台即可保持聚合口径一致。
