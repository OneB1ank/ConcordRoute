...
func TestUsageLogEffectiveModelIndexesMigration(t *testing.T) {
	content, err := FS.ReadFile("251_add_usage_log_effective_model_indexes_notx.sql")
	...
	require.Contains(t, sql, "COALESCE(NULLIF(BTRIM(model), ''), NULLIF(BTRIM(requested_model), ''), '')")
	require.Contains(t, sql, "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_usage_logs_effective_upstream_model_created")
	require.Contains(t, sql, "COALESCE(NULLIF(BTRIM(upstream_model), ''), model)")
}
